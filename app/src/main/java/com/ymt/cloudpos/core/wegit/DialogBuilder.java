...
/**
 * Created by zhangjifeng on 2018/4/18.
...
public class DialogBuilder {
    ...
    private int sureColor = -1, cancelColor  = -1, titleColor = -1, messageColor = -1;
    ...
    public DialogBuilder(Context context, int themeResId) {
        this(context, themeResId, ((LayoutInflater) context
                .getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(R.layout.custom_dialog, null));
    }
    //自定义layout用这个
    ...
    public DialogBuilder setSureColor(int sureColor) {
    ...
    public DialogBuilder setMessageColor(int messageColor) {
    ...
    public Dialog build() {
        ...
        setColor(titleColor, R.id.title);
        setColor(messageColor, R.id.message);
        setColor(cancelColor, R.id.cancel);
        setColor(sureColor, R.id.sure);
    ...
    private void setColor(int color, int id) {
        if ( color != -1) {
            TextView textView = (TextView) layout.findViewById(id);
